		  ...
		  struct mx7_calibration *calib_param)
{
	...
	/* Assert DDR Controller core reset and DDR PHY reset */
	writel(SRC_DDRC_RCR_DDRC_CORE_RST_MASK |
	       SRC_DDRC_RCR_DDRC_PRST_MASK, &src_regs->ddrc_rcr);
	udelay(500);
	/* Clear DDR Controller preset and DDR PHY reset */
	...
	for (i = 0; i < calib_param->num_val; i++) {
		...
		if (i == 2) {
			/* 
			 * long calibration started, now
			 * wait until calibration is done.
			 */
			do {
				u32 zq_done =
				    readl(&ddr_phy_regs->zq_con1) & 0x1;
				udelay(10);
				if (zq_done)
					break;
			} while (1);
...
unsigned int imx_ddr_size(void)
{
	...
	bits += field_val == 0xf ? 2 : field_val == 0x3 ? 1 : 0 ;
